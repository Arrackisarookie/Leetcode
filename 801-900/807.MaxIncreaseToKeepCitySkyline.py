...
#   [0, 3, 1, 0] ]
#
# The skyline viewed from top or buttom is: [9, 4, 8, 7]
...
class Solution:
    def maxIncreaseKeepingSkyline(self, grid) -> int:
        addition = 0
        max_horizon = []
        max_vertical = [0] * len(grid[0])
        for line in grid:
            max_horizon.append(max(line))
            for i, item in enumerate(line):
                max_vertical[i] = max(max_vertical[i], item)

        for i, line in enumerate(grid):
            for j, item in enumerate(line):
                addition += min(max_horizon[i], max_vertical[j]) - item
